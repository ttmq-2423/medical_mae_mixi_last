...
from model_ConvNeXt_Swin import ConvNeXt_Swin
import transformers
from transformers import MobileViTFeatureExtractor, MobileViTForImageClassification

from engine_finetune import train_one_epoch, evaluate_chestxray
from util.sampler import RASampler
# Import FusedAdam if available, otherwise use standard AdamW
try:
    from apex.optimizers import FusedAdam
except ImportError:
    print("FusedAdam not available, will use standard optimizers")
    FusedAdam = None

from libauc import losses
from torchvision import models
import timm.optim.optim_factory as optim_factory
from sunsilk.model.convnext_swin import convnext_base_swin4
...
def main(args):
    for arch in ArchList_VIT:
        ...
        model = arch_functions[arch](num_classes=5)
        ...
        elif args.optimizer == 'fusedlamb' and FusedAdam is not None:
            optimizer = FusedAdam(param_groups, lr=args.lr)
        else:
            # Default to AdamW if specified optimizer not available
            optimizer = torch.optim.AdamW(param_groups, lr=args.lr)

        loss_scaler = NativeScaler()

        # Set criterion based on dataset type
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if args.output_dir and (epoch % args.eval_interval == 0 or epoch + 1 == args.epochs):
                ...
                print(f'Max Average AUC: {max_auc:.4f}')
